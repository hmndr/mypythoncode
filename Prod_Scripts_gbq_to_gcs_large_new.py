...
for i in range(1,col):
    ...
    cmd= 'bq extract --destination_format CSV --field_delimiter tab '+project_id+':'+dataset_name+'.'+table_name+' '+path+target_table_name+'-*.csv'
    os.system(cmd)
    ...
    if count <33:		  
        ...
        os.system('gsutil mv '+VM_path+target_table_name+'.csv '+path)
        file_object2 = open(VM_path+'check.txt')
        for line in file_object2:
            match_obj = re.match( '%s(.*).csv'%(path),line)
            if(match_obj):
